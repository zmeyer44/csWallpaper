...
function Combine() {
  ...
  const downloadImage = () => {
    ...
    link.download = "DemonicCombined.png";
    link.href = url;
    link.click();
  };
  ...
  const handleSearch = async () => {
    ...
    skulls.forEach(async (skull, index) => {
      return await fetch(
        `https://api.opensea.io/api/v1/asset/0xd3cf04d7a5513ce8148790d90d91361476f5da94/${skull}`,
        options
      )
    ...
    <>
      ...
      <div className="bg-dark min-h-screen flex flex-col justify-center items-center px-3">
        <h1 className="mb-4 text-6xl title text-center text-white sm:mb-6 md:mb-10 md:text-7xl lg:mb-12 lg:text-8xl">
          Demonic <span className="text-red-700">Combine</span>
        </h1>
        {!generate ? (
